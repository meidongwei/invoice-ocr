...
在 macOS 上双击 `BuildToApp.command` 即可将应用打包为可双击的 `发票识别.app`。

### 1. 打包

1. 在 macOS 上双击 `BuildToApp.command`
2. 等待完成（首次需联网安装依赖）
3. 产物位于 `dist/发票识别.app`

### 2. 分发给普通用户

将 `发票识别.app` 压缩为 zip 发给对方（对方无需安装 Python）：

```
发票识别.app     ← 右键「压缩」成 zip 后发送
```

只需发送 `发票识别.app`，不用发送 `dist/` 里多余的 `发票识别/` 文件夹——那只是打包中间产物，其内容已完整包含在 `.app` 中。

> 因为 app 未做代码签名与公证，对方首次打开可能被拦截，解决办法见下方「🍎 macOS 使用提示」。

### 3. 打包失败排查

在终端进入项目目录运行 `bash BuildToApp.command`，把报错信息发回即可。

常见错误：`Read timed out` / 依赖下载超时
- 这是下载 Python 依赖超时，不是程序代码错误
- 若失败，换一个网络或手机热点后重新双击 `BuildToApp.command`
